...
- FRF / АЧХ: `frf_analysis` (численное ядро без GUI, среднее опор до H1,
  ...
  Выбор опор и откликов открывается из своих контролов компактным немодальным
  popup со списком флажков в стиле первого варианта: изменения применяются сразу,
  popup остаётся открытым до клика вне него или повторного нажатия контрола.
  Роли не пересекаются; каналы противоположной роли недоступны. У нового
  документа роли изначально не выбраны.
